...
def analyze_pcap(pcap_file):
    try:
        ...
        if icmp_ip_count:
            ...
            max_ip = [ip for ip, count in icmp_ip_count.items() if count == max_count]
            print("ICMP Echo Request - IP address with the most packets:", max_ip, "(Packets:", max_count, ")")
        else:
            print("ICMP Echo Request - No packets found.")

        if tcp_syn_ip_count:
            max_count = max(tcp_syn_ip_count.values())
            max_ip = [ip for ip, count in tcp_syn_ip_count.items() if count == max_count]
            print("TCP SYN Scan - IP address with the most packets:", max_ip, "(Packets:", max_count, ")")
        else:
            print("TCP SYN Scan - No packets found.")

        if tcp_ack_ip_count:
            max_count = max(tcp_ack_ip_count.values())
            max_ip = [ip for ip, count in tcp_ack_ip_count.items() if count == max_count]
            print("TCP ACK Scan - IP address with the most packets:", max_ip, "(Packets:", max_count, ")")
        else:
            print("TCP ACK Scan - No packets found.")

        if udp_ip_count:
            max_count = max(udp_ip_count.values())
            max_ip = [ip for ip, count in udp_ip_count.items() if count == max_count]
            print("UDP Scan - IP address with the most packets:", max_ip, "(Packets:", max_count, ")")
        else:
            print("UDP Scan - No packets found.")

        if arp_ping_ip_count:
            max_count = max(arp_ping_ip_count.values())
            max_ip = [ip for ip, count in arp_ping_ip_count.items() if count == max_count]
            print("ARP Ping Scan - IP address with the most packets:", max_ip, "(Packets:", max_count, ")")
        else:
